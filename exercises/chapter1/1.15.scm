...
; by making use of the approximation sin x  x if x is sufficiently small, and
...
; It is logarithmic in number of steps O(log(a)) and the memory required.  If
; we triple the input, it will take only one more step; triple it again, only
; one more step again; and so on.  This is way better than linear.
